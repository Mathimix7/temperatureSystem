...
class WifiManager:
    ...
    def __init__(self, display, ssid = 'WifiPY', password = 'wifimanager'):
    ...
    def __WebServer(self):
        ...
        if DNSserver.MicroDNSSrv.Create( {
          "mathimix.com" : "192.168.4.1",
        ...
        while True:
            ...
            try:
                ...
                print('Something went wrong! Reboot and try again. -', e)
                return
            finally:
                self.client.close()
    ...
    def __HandleRoot(self):
        ...
        self.client.sendall("""
                        ...
                        <p><label for="name">Name:&nbsp;</label><input type="name" id="name" name="name"></p>
    ...
    def __HandleConfigure(self):
        ...
        if match:
            ...
            passwordAndName = match.group(2).decode('utf-8').replace('%3F', '?').replace('%21', '!')
            password, name = passwordAndName.split("&name=")
            with open("name.dat", 'w') as myfile:
                myfile.write(name)
            if len(ssid) == 0:
                self.__SendResponse("""<p>SSID must be providaded!</p><p>Go back and try again!</p>""", 400)
            elif len(name) == 0:
                self.__SendResponse("""<p>Name must be providaded!</p><p>Go back and try again!</p>""", 400)
