try:
    ...
    stop_button = machine.Pin(12, machine.Pin.IN, machine.Pin.PULL_UP)
    ...
    spi.init()
    lora = LoRa(spi, cs=machine.Pin(5, machine.Pin.OUT), rx=machine.Pin(26, machine.Pin.IN), frequency=433.0, tx_power=17, bandwidth=125000, spreading_factor=7, coding_rate=5, preamble_length=8, crc=True)
    lora.set_implicit(False)
    # Global variable to store received LoRa data
    lora_data_buffer = None
    ...
    UPDATE_CURRENT_INTERVAL_MS = 5000
    last_update_current_ms = time.ticks_ms()
    ...
    def on_lora_recv(payload):
        global lora_data_buffer
        try:
            lora_data_buffer = payload
            print("LoRa received:", payload)
        except Exception as e:
            print("Error in LoRa callback:", e)

    # Set up LoRa callback and start receiving
    lora.on_recv(on_lora_recv)
    lora.recv()

    def formatLoRadata(data):
        try:
            if isinstance(data, bytes):
                data = data.decode("utf-8")
            temp, hum, voltage = data.split(":")
            temp = int(temp[1:])/10
            hum = str(int(hum[1:])/10)
            hum = hum.split(".")
            hum = hum[0] + "." + hum[1][:1]
            voltage_value = int(voltage[1:])  # Extract voltage value from V4149 format
            return float(temp), float(hum), voltage_value
        except Exception as e:
            print("Error parsing LoRa data:", data, e)
            return None
    ...
    while True:
        ...
        if wm.is_connected() and get_api_status():
            ...
            now_ms = time.ticks_ms()
            if time.ticks_diff(now_ms, last_update_current_ms) >= UPDATE_CURRENT_INTERVAL_MS:
                last_update_current_ms = now_ms
                try:
                    headers = {
            ...
            if lora_data_buffer is not None:
                try:
                    parsed = formatLoRadata(lora_data_buffer)
                    if parsed:
                        tempOutside, humOutside, batteryValue = parsed
                        lora_data_buffer = None  # Clear buffer after processing
            ...
            # Display battery icon if we have battery data
            if batteryValue is not None:
                battery_level = icons.getBatteryLevel(batteryValue)
                icons.batterySymbol(display2, 110, 0, battery_level)
            else:
            ...
            if str(timeNow[4] * 60 + timeNow[5]).endswith("0"):
                if status == "false":
                    ...
                    try:
                        ...
                        print("Did not send battery value", e)
            else:
                if status == "true":
                    with open("sent.dat", "w") as f:
                        f.write("false")
