    ...
    if wm.is_connected() and apiON:
        ...
        if second % 5 == 0:
            try:
                ...
                response = urequests.post(f"http://{ip}:{port}/updateCurrent/{objectID}", json=headers)
            except:
        ...
        if not wm.is_connected():
            display.fill_rect(x=0,y=0,w=100,h=19,color=0)
            timeNow = rtc.datetime()
            second = timeNow[6]
            if second % 5 == 0:
                if corf == "F":
            ...
            temperatureNow = temperatureNow.split(".")
            temperatureNow = temperatureNow[0] + "." + temperatureNow[1][:1]
            display.text(f"{temperatureNow}{corf}", 0, 0)
            display.text(f"{round(sensor.relative_humidity, 1)}%", 50, 0)
            display.hline(0,14,128,0xffff)
            display.show()
