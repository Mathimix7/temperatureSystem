...
class SH1106(framebuf.FrameBuffer):
    ...
    def pixel(self, x, y, color=0xffff):
        if color is None:
            super().line(x, y, x, y, color)
        else:
...
class SH1106_I2C(SH1106):
    ...
    def _newline(self):
        ...
        self.text_row += height
        self.text_col = 0
        margin = self.height - (self.text_row + height)
        if margin < 0:
            self.scroll(0, margin)
            self.text_row += margin
    
    def printstring(self, col, row, text, font):
        self.text_row = row
        self.text_col = col
        self.font = font
        if self.font.hmap():
            self.map = framebuf.MONO_HMSB if self.font.reverse() \
    ...
    def _printchar(self, char, invert=False):
        ...
        if self.text_row + char_height > self.height:
            self._newline()
        if self.text_col + char_width > self.width:
            self._newline()
        buf = bytearray(glyph)
        if invert:
        ...
        self.blit(fbc, self.text_col, self.text_row)
        self.text_col += char_width
